...
\begin{align}
  \left<P_{\mathrm{att}}\right> &= \frac{1}{V} \int_V P(a, z) \, dV = \frac{1}{\pi d^2} \int_0^d 2 \pi a P_{\mathrm{att}}(a) \, da \label{eq:P-aveaz}
\end{align}
...
この結果を$\psi$について平均化したものを実際の計算では使っている。
その分布を表したものが図\ref{fg:Distribution_of_Initial_Position}である。
\begin{figure}[h]
  \centering
...
\end{figure}
以下aで積分を行う際に、この分布を重み付として積分を行うことがある。
この重みがaで積分する際のaの役割を担うことになる。つまりは、aが一様であるように積分をしていたところをこの分布関数に従うように積分を変更するということである。
...
\begin{align}
  \left< \frac{dP(\theta)}{d\theta} \right> &= \frac{1}{V} \int_V \frac{dP(a, \theta)}{d\theta} \exp \left( -\frac{L - z}{\cos \theta \Latt} \right) \, dV \nonumber \\
  &= A(\theta) \int_0^d 2 \pi a \frac{dP(a, \theta)}{d\theta}  \, da. \label{eq:dPdtheta-aveV}\\
...
\begin{align}
   \frac{dP(a, \theta)}{d \cos \theta} &= \frac{dP(a, \theta)}{d \theta} \frac{d \theta}{d \cos \theta} = -\frac{1}{\sin \theta}\frac{dP(a, \theta)}{d \theta} = -\frac{1}{2 \pi} \int_{\Phi(\theta)}  \, d \phi, \label{eq:dPdcostheta} \\
     \left< \frac{dP(\theta)}{d\cos\theta} \right> &= \frac{1}{V} \int_V \frac{dP(a, \theta)}{d\cos\theta} \exp \left( -\frac{L - z}{\cos \theta \Latt} \right) \, dV = A(\theta) \int_0^d 2 \pi a \frac{dP(a, \theta)}{d\cos\theta} \, da \label{eq:dPdcostheta-aveV}
\end{align}
...
\begin{align}
   \left< \frac{dP(\theta)}{d\ell} \right> &= \frac{1}{V} \int_V \frac{dP(a, \theta)}{d\cos\theta} \frac{d\cos\theta}{d\ell} \exp \left( -\frac{L - z}{\cos \theta \Latt} \right) \, dV \nonumber \\
   &= \frac{1}{V} \exp \left( -\frac{\ell}{\Latt} \right)  \int_0^L \frac{z-L}{\ell^2} \,  dz \int_0^d 2 \pi a  \frac{dP(a, \theta)}{d\cos\theta}  \, da \nonumber \\
  &= B(\theta) \int_0^d 2 \pi a  \frac{dP(a, \theta)}{d\cos\theta}  \, da
...
これの$\theta $に関する平均化を行う。
\begin{equation}
  \left< \frac{dP}{dt} \right>_\theta = \int_{0}^{\pi/2}\sin \theta \frac{dP(a, z)}{dt} d\theta = \frac{c'}{2 \pi} \int_{0}^{\pi/2} \int_{\Phi(\theta)} \sin \theta \frac{\cos^2\theta}{L-z} d \phi d\theta
...
\begin{equation}
  \frac{dP}{d\frac{1}{\cos\theta}} = \frac{\cos^2\theta}{\sin\theta}\frac{dP}{d\theta} = \frac{\cos^2\theta}{\sin\theta}A(\theta) \int_0^d 2 \pi a \frac{dP(a, \theta)}{d\theta}da
  \label{dPdsec}
\end{equation}
これを$\theta$で平均化すると
\begin{equation}
  \left<\frac{dP}{d\frac{1}{\cos\theta}}\right>_{\theta} = \cos^2 \theta A(\theta) \int_0^d 2 \pi a \frac{dP(a, \theta)}{d\theta}da
  \label{dPdsec_ave}
\end{equation}
