...
\end{align}
ここで、任意の$a$と$\theta$に対して、$\phi$に関する積分の区間は、式\eqref{eq:criticalangle1}を$\phi$に適用して得られる$\Phi(\theta)$の範囲に制限される。
...
\begin{align}
  \frac{dP(a, z)}{dl} = \frac{dP(a, z)}{d\cos\theta} \frac{d\cos\theta}{dl} = - \frac{L-z}{l^2} \frac{dP(a, z)}{d\cos\theta} = \frac{1}{2 \pi} \frac{L-z}{l^2} \int_{\Phi(\theta)}
\end{align}
